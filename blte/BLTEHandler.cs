...
namespace blte
{
    ...
    class BLTEHandler
    {
		...
		public void ExtractData(string path, string name, int size)
        {
			...
			if (compDataOffset == 0) { //if this is 0 there's just one chunk with no chunk-header
				chunkCount = 1;
				...
				chunks [0].hash = null;
				chunks [0].decompSize = Int16.MaxValue;
		    ...
		    for (int i = 0; i < chunkCount; ++i)
            {
				...
				Console.WriteLine ("Now we are at {0}", reader.BaseStream.Position);
